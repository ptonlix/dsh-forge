...
import { checkDocumentation } from '../docs/check.ts';
import { writeBilingualSidecars } from '../docs/bilingual.ts';
...
export function main(command = process.argv[2], profileName = process.argv[3]): number | void {
  ...
  if (command === 'docs:check') {
    json({ valid: true, ...checkDocumentation(root) });
    return 0;
  }
  if (command === 'docs:pair') {
    const arguments_ = process.argv.slice(3);
    const write = arguments_.includes('--write');
    const files = arguments_.filter((argument) => argument !== '--write');
    if (!write) throw new Error('docs:pair 需要 --write，避免意外改写 sidecar');
    json({ valid: true, ...writeBilingualSidecars(root, files) });
    return 0;
  }
  process.stderr.write(
    '用法: profile:resolve [profile] | profile:verify [profile] | dump-config [profile] | catalog:verify | package:inspect [profile] | release:gate [profile] | docs:check | docs:pair --write [英文文档]\n',
  );
  return 2;
}
