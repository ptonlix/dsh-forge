...
The "Storage space" Settings page renders only read-only facts projected by the generation-owned storage capability. Storage is a private capability that third-party bundles cannot obtain: the main process scans exactly the launcher-resolved DSH Home and Electron `userData` roots, classifies usage into cache, sessions, and other, and sums application bytes; the disk share is based on the DSH Home volume, and a `userData` directory on another volume is merely flagged, never merged. Scanning runs only when the Settings page opens or an explicit refresh is requested and shares one generation lease with cleanup. Cache and session cleanup require a native main-process confirmation and delete only fixed allow-listed targets; session confirmation must warn that the DSH Home may be shared with other DSH processes, and credentials or the managed profile are never removable. The page reads snapshots and requests cleanup through a fixed, parameter-less Typert Remote and never receives absolute paths, file names, or directory candidates.
...
Full-package OTA is owned jointly by `apps/desktop` and the private `@dsh-forge/desktop-services-local/launcher`. A packaged app reads Windows, macOS, and Ubuntu AppImage entries from the fixed GitHub Release `version.json`, compares SemVer before `build`, and lets a generation-owned `UpgradeCoordinator` check silently at readiness and every 12 hours. Only a Settings-initiated fresh check that still finds an update and receives native confirmation downloads a full package and exits for helper preparation. During download, the fixed Typert Remote projects byte counts and a percentage only when the response provides a valid total, so the Settings page can render determinate or indeterminate progress without receiving a URL, path, command, or token. A controlled Windows staging `cmd.exe` runner waits for Electron to exit so NSIS can replace the application binary, then it explicitly starts the updated Windows executable, macOS bundle, or Ubuntu AppImage and waits for a random-token receipt written only after the new generation is fully ready; command success alone does not complete the upgrade. A missing receipt retains recovery evidence and restores a replaced macOS or Ubuntu application. macOS cleanup runs only after the receipt and is best-effort, so a failed DMG detach cannot misreport a completed restart as failed. Ubuntu releases and upgrades only a writable Ubuntu 22.04+ AppImage; other Linux launch methods are unsupported. This channel intentionally has no package digest, manifest signature, or runtime trust-root verification and must not be described as a general trusted update channel.
