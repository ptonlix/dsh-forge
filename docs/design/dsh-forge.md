...
The compiler resolves runtime compatibility, bundle manifests, peer dependencies, static catalog entries, exact Git commits, lifecycle-script authorization, and the dependency closure. It calculates an input digest before writing profile and release evidence. The composer runs the real DSH loader to produce a configuration dump; a healthy dump is required for verification and packaging.
