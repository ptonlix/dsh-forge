...
- Linux 的唯一安装包格式为 AppImage；工作流、打包参数和 Release 附件不得生成或上传 `.deb`。
...
- 对 tag 发布增加版本与 profile 身份检查；只有所有声明目标证据齐全且
  `release:gate` 通过时才创建 GitHub Release。后续变更要求 macOS universal 产物完成
  Developer ID 签名、公证与 stapling；Windows 与 Linux 未签名产物仍明确标记为 `unsigned-smoke`。
...
- Windows Authenticode 不包含在当前实现；完整安装包 OTA 与 macOS 签名/公证由
  `add-full-package-ota-and-macos-signing` 变更定义。
...
- 可能更新 `docs/engineering/foundation-verification.md` 与发布参考，记录 CI 证据、平台
  覆盖和签名缺口。
- GitHub Actions 需要仓库 `contents: write`（仅满足完整门禁的 tag Release job）；Apple 凭据只由
  tag macOS package job 使用，普通构建 job 与 release job 不读取它们。
