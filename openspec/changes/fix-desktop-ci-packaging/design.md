...
17. Windows Builder 的 ZIP 阶段两次在新下载 `7zip-win-x64.tar.gz` 后仍以 `ENOENT` 找不到
    ...
    cwd、Node 架构和启动错误，便于区分 runner 清理/隔离与 Builder 子进程上下文问题。Windows 的 `packageRoot` 固定为
    `win-unpacked` 目录，不能把其中的 `.exe` 文件作为 `--prepackaged` 或 Builder cwd。Builder 不再自行下载
    或解压工具；工作流也不缓存其工具目录。
